...
def llama_3b_nll_fn(model, input_arr, target_arr, settings:SerializerSettings, transform, count_seps=True, temp=1, cache_model=True):
...
def llama_3b_completion_fn(
    ...
    avg_tokens_per_step = len(tokenize_fn(input_str, model=model)['input_ids']) / len(input_str.split(settings.time_sep))
